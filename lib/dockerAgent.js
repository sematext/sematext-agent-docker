      ...
      start: function (agent) {
        this.dockerListener = function (stats) {
          ...
          if (stats.dockerId) {
            ...
            if (ignoreImageVersion && imageName && imageName.indexOf('sha256') !== 0) {
            ...
            if (process.env.SPM_CONTAINER_COUNT_PER_IMAGE === undefined || process.env.SPM_CONTAINER_COUNT_PER_IMAGE === 'false') {
              SpmAgent.Logger.debug(agent.formatLine(ccMetric))
              if (stats.name === 'count' && stats.filters === undefined) {
            ...
            if (process.env.SPM_CONTAINER_COUNT_PER_IMAGE === 'true' && stats.name === 'count' && stats.filters !== undefined) {
              if (stats.name === 'count' && stats.filters !== undefined) {
                ccMetric.filters = stats.filters
                agent.addMetrics(ccMetric)
                SpmAgent.Logger.debug('container count = ' + stats.value + ' image:' + ccMetric.filters)
              }
            }
            if (stats.name === 'count' && stats.filters === undefined) {
              containerCount = stats.value
              global.spmSenderUrlParameters = '&containerCount=' + containerCount
              SpmAgent.Logger.debug('container count (total)  = ' + stats.value)
            }
