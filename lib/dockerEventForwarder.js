    ...
    try {
      if (dockerEvent.Type) {
        ...
        if (dockerEvent.Actor && dockerEvent.Actor.Attributes) {
          ...
          if (dockerEvent.Actor.Attributes.name) {
      ...
      var msg = {
        ...
        tags: [ 'docker', process.env.SPM_REPORTED_HOSTNAME || process.env.HOSTNAME || os.hostname(), dockerEvent.status, dockerEvent.id.substring(0, 12) ]
      }
      if (self.logsene) {
