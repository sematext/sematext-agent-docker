...
function DockerEventForwarder (docker) {
  ...
  if (process.env.LOGSENE_TOKEN || process.env.LOGSENE_STATS_TOKEN) {
    var Logsene = require('logsene-js')
    this.logsene = new Logsene(process.env.LOGSENE_TOKEN || process.env.LOGSENE_STATS_TOKEN, 'dockerEvent')
  ...
  this.forwardEvent = function (dockerEvent) {
    try {
      ...
      var msg = {
        _type: 'dockerEvent',
        dockerEventStatus: '' + dockerEvent.status,
        ...
        container_id: (dockerEvent.id || '').substr(0, 12),
