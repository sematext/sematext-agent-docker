...
function getLogseneToken (err, info) {
  ...
  if (!err) {
    ...
    // exclude sematext-docker-agent logs
    // avoid endless loops ...
    if (info.Config && info.Config.Image && /sematext-agent-docker/.test(info.Config.Image)) {
    ...
    if (info.LOGSENE_ENABLED === '0' || info.LOGSENE_ENABLED === 'false' || info.LOGSENE_ENABLED === 'no') {
      ...
      console.log('Container ' + info.Id + ' ' + info.Name + ' setting LOGSENE_ENABLED=true')
    }
