...
function replaceDockerHeaders (txt) {
  // temporary workaround for docker-logshose issues
  var rv = txt
  try {
    if (txt && txt.lenght > 8) {
      var txtBuffer = new Buffer(txt)
      var header = txtBuffer.slice(0, 8)
      var body = txtBuffer.slice(8, txtBuffer.length)
      if (header.slice(1, 4).toString() === '\u0000\u0000\u0000') {
        rv = body.toString()
      } else {
...
DockerLogsene.prototype.logLine = function (logline, data, next) {
  ...
  var lineParser = {
    ...
    parse: function () {
      ...
      self.getLogObject(
      replaceDockerHeaders(this.line),
      ...
      function (err, logObject) {
        ...
        if (logObject) {
          ...
          if (!logObject['_type']) {
            ...
            // logObject['_type'] = logObject.container_id
          }
