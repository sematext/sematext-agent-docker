...
var errorRegex = /error|exception/i
...
function logToLogsene (level, message, data) {
  ...
  var logger = null
  if (dockerInspect) {
    logger = getLogger(dockerInspect.LOGSENE_TOKEN || process.env.LOGSENE_TOKEN)
    attachLabelsToLogs(dockerInspect, data)
  } else {
    dockerInspectHelper.getLogseneTokenByContainer(data.container_id, function (err, info) {
...
function DockerLogsene () {
  ...
  if (process.env.SEVERITY_ERROR_PATTERN) {
    errorRegex = new RegEx(process.env.SEVERITY_ERROR_PATTERN)
  }
  if (process.env.SEVERITY_WARNING_PATTERN) {
    warningRegex = new RegEx(process.env.SEVERITY_WARNING_PATTERN)
  }
  this.logCount = 0
...
DockerLogsene.prototype.initDockerEventHandlers = function () {
  ...
  this.dockerEvents.on('connect', function () {
    ...
    this.dockerEvents.on('start', function (dockerEvent) {
      dockerInspectHelper.getLogseneTokenByContainer(dockerEvent.id.substr(0, 12), function (err, info) {
        if (!err && info) {
...
DockerLogsene.prototype.connect = function () {
  ...
  try {
    this.lh.on('error', this.reconnect)
    this.lh.pipe(self.logStream).on('error', this.reconnect)
  } catch (ex) {
...
DockerLogsene.prototype.enrichAndLog = function (err, logObject) {
  if (err && !logObject) {
    ...
    if ((Date.now() - logObject['@timestamp'].getTime()) < (maxAge)) {
      var severity = (logObject.level || logObject.lvl || logObject.severity || 'info')
      if (autodetectSeverity && logObject.message && severity === 'info') {
        // detect severity
        var testString = String(logObject.message).substring(0, 256)
