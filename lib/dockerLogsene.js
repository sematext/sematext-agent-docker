...
DockerLogsene.prototype.connect = function () {
  ...
  try {
    ...
    this.lh.pipe(self.logStream).on('error', function (){
      console.log('Docker reconnect: because of log stream error')
      setTimeout(self.reconnect.bind(self), 10000)
    })
    process.on('uncaughtException', function (err) {
      if (String(err).indexOf('docker')) {
        
          this.reconnectInProgress = true
          SpmAgent.Logger.log('error', 'Docker reconnect: reconnect because of an error in Docker communication:' + (err.message || err))
          setTimeout(self.reconnect.bind(self), 10000)
        }
    })
    if (this.iv) {
...
DockerLogsene.prototype.reconnect = function (err) {
  var self = this
  SpmAgent.Logger.log('error', 'reconnect: waiting for Docker socket error:' + err)
  var doReconnect = function () {  
      try {
        SpmAgent.Logger.log('debug', 'reconnect to Docker socket ')
        self.logStream = null
  ...
  if (!self.iv) {
    self.iv = setInterval(doReconnect, 1000)
  }
}
