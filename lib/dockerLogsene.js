...
function DockerLogsene () {
  ...
  setInterval(function () {
    var logInfoString = 'Log count: \t' + self.logCount + '\n'
    logInfoString = logInfoString + 'Logs shipped:\t' + logsShipped + '\n'
    logInfoString = logInfoString + 'Log bulk req. failed:\t' + httpFailed + '\n'
    logInfoString = logInfoString + 'Log bulk re-transmits:\t' + httpRetransmit + '\n'
    ...
    logger.log('info', logInfoString,
        {type: 'sda-logging-stats',
         logCount: Number(self.logCount),
         logsShipped: Number(logsShipped),
         httpFailed: Number(httpFailed),
         httpRetransmit: Number(httpRetransmit)})
    self.logCount = 0
    logsShipped = 0
    httpFailed = 0
    httpRetransmit = 0
...
DockerLogsene.prototype.enrichAndLog = function (err, logObject) {
  if (err && !logObject) {
    ...
    if (!logObject['_type']) {
      logObject['_type'] = this.chunk.image.replace(/[\W]/gi, '_')
    }
    ...
    if ((Date.now() - logObject['@timestamp'].getTime()) < (90 * 24 * 60 * 60 * 1000)) {
      logToLogsene(
        ...
        logObject.message || logObject.msg || logObject.MESSAGE,
        logObject)
    }
  }
}
...
DockerLogsene.prototype.logLine = function (messageText, data, next) {
  ...
  var logContext = {
    ...
    chunk: data,
    line: (process.env.REMOVE_ANSI_ESCAPE_SEQ === 'enabled') ? messageText.replace(
