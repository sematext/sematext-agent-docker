...
var GOOGLE_PAUSE_REGEX = /google_containers\/pause/
var opts = {
...
function countPauseContainers (cb) {
  docker.listContainers(function (err, info) {
    if (err) {
      ...
      if (info) {
        var k8sPause = info.filter(function (c) { return GOOGLE_PAUSE_REGEX.test(c.Image) && c.State === 'running' })
        if (k8sPause) {
          cb(null, k8sPause.length)
        } else {
          cb(null, 0)
        }
      }
  ...
  countPauseContainers(function (err, pauseCount) {
    ...
    docker.info(function dockerInfoHandler (err, data) {
      ...
      if (data && data.ContainersRunning) {
        ...
        containerCount = Object.keys(cMetrics).length - k8sPauseCount
