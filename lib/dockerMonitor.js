...
// remove inactive containers every 3 minutes (6 * 20 sec collection interval)
setInterval(function cleanUpMetrics () {
  var now = Date.now()
  Object.keys(cMetrics).forEach(function removeEntry (entry) {
    if (cMetrics[entry].lastUpdate && ((now - cMetrics[entry].lastUpdate) > 6 * config.collectionInterval)) {
      delete cMetrics[entry]
    }
  })
}, 6 * config.collectionInterval + 1000)
...
function processStats (chunk, enc, cb) {
  ...
  if (process.env.SPM_LOG_LEVE==='debug') {
    console.log('Docker API container stats:')
    console.log(stats)
  }
  var metric = {}
