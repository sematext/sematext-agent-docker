...
function containerCountQuery () {
  docker.info(function dockerInfoHandler (err, data) {
    ...
    if (data && data.ContainersRunning) {
      containerCount = Number(data.ContainersRunning)
    } else {
      containerCount = Object.keys(cMetrics).length
      // console.log('error', 'error dockerInfo.ContainersRunning not reported ')
    }
  })
}
