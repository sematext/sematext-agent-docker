...
var tokens = 0
if (process.env.LOGSENE_STATS_TOKEN || process.env.SPM_TOKEN || process.argv[2] && process.argv[2].length > 30) {
  tokens++
  DockerMonitor()
} else {
...
process.on('uncaughtException', function (err) {
  ...
  errorCounter++
  if (errorCounter > 1) {
    // console.log('more than N uncaught errors -> exit.')
    process.exit(2)
  }
//
})
...
function log (level, message) {
  var message = (new Date).toISOString() + ' - ' + level + ' - ' + message
  if (level === 'ERROR') {
